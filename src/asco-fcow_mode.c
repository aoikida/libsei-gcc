...
struct asco {
    ...
    abuf_t*   abuf[2]; /* a copy-on-write buffer      */
...
asco_init()
{
    ...
    assert(asco);
    asco->abuf[0] = abuf_init(100000);
    asco->abuf[1] = abuf_init(100000);
...
{
    assert(asco);
    abuf_fini(asco->abuf[0]);
    abuf_fini(asco->abuf[1]);
...
asco_commit(asco_t* asco)
{
    ...
    abuf_cmp_heap(asco->abuf[0], asco->abuf[1]);
    abuf_clean(asco->abuf[0]);
    abuf_clean(asco->abuf[1]);
...
#define ASCO_WRITE(type) inline                                         \
    ...
    {                                                                   \
        ...
        DLOG3("asco_write_%s(%d): %p <- %llx\n", #type, asco->p,        \
              addr, (uint64_t) value);                                  \
        abuf_push_##type(asco->abuf[asco->p], addr, *addr);             \
        *addr = value;                                                  \
    }
ASCO_WRITE(uint8_t)
ASCO_WRITE(uint16_t)
